...
static void predictEKF(const uint8_t model_index, const Vector3f delta_ang, const float delta_ang_dt,
...
static void YawEstPredictCovariance(const e_matrix_t* state,
									e_matrix_t* P,
									const Vector2f d_vel, const float d_vel_var,
									const float d_ang, const float d_ang_var);

static void YawEstComputeMeasurementUpdate(e_matrix_t* P, const float vel_obs_var,
                                    const float epsilon,
                                    e_matrix_t* S_inv,
                                    float* S_det_inv,
                                    e_matrix_t* K );
...
void EKFGSF_yaw_predict(const imuSample_t imu_sample, bool in_air)
{
	...
	for (uint8_t model_index = 0; model_index < N_MODELS_EKFGSF; model_index ++) {
		predictEKF(model_index, imu_sample.delta_ang, delta_dt,
			   imu_sample.delta_vel, delta_dt, in_air);
	}
}
...
void EKFGSF_yaw_reset(void)
{
	...
	_ekfgsf_yaw->_gsf_yaw_variance = INFINITY;
}
			    ...
			    const Vector3f delta_vel, const float delta_vel_dt, bool in_air)
{
	...
	YawEstPredictCovariance(_ekfgsf_yaw->_ekf_gsf[model_index].X, _ekfgsf_yaw->_ekf_gsf[model_index].P,vel_b, d_vel_var, daz, d_ang_var);
...
bool EKFGSF_yaw_getLogData(float *yaw_composite, float *yaw_variance, float yaw[N_MODELS_EKFGSF],
...
bool EKFGSF_yaw_isActive(void){ 
	bool active = _ekfgsf_yaw->_ekf_gsf_vel_fuse_started;
	return active;
}
...
static void YawEstPredictCovariance(const e_matrix_t* state,
  ...
  const float _tmp3 = P->mat[0][2] + P->mat[2][2] * _tmp2;
  const float _tmp4 = Sq(_tmp0) * d_vel_var + Sq(_tmp1) * d_vel_var;
  const float _tmp5 = _tmp0 * d_vel.x - _tmp1 * d_vel.y;
  const float _tmp6 = P->mat[1][2] + P->mat[2][2] * _tmp5;
  const float _tmp7 = Sq(d_ang) + 1;



	e_matrix_t p_tmp;
	InitMatrix(&p_tmp,3,3,0.0f);

	p_tmp.mat[0][0] = P->mat[0][0] + P->mat[2][0] * _tmp2 + _tmp2 * _tmp3 + _tmp4;
	p_tmp.mat[1][0] = 0.0f;
	p_tmp.mat[2][0] = 0.0f;
	p_tmp.mat[0][1] = P->mat[0][1] + P->mat[2][1] * _tmp2 + _tmp3 * _tmp5;
	p_tmp.mat[1][1] = P->mat[1][1] + P->mat[2][1] * _tmp5 + _tmp4 + _tmp5 * _tmp6;
	p_tmp.mat[2][1] = 0.0f;
	p_tmp.mat[0][2] = _tmp3 * _tmp7;
	p_tmp.mat[1][2] = _tmp6 * _tmp7;
	p_tmp.mat[2][2] = P->mat[2][2] * Sq(_tmp7) + d_ang_var;

	CopyMatrixValues(&p_tmp,P);
...
static void YawEstComputeMeasurementUpdate(e_matrix_t* P, const float vel_obs_var,
	...
	const float _tmp0 = P->mat[1][1] + vel_obs_var;
	const float _tmp1 = P->mat[0][0] + vel_obs_var;
	const float _tmp2 = -P->mat[0][1] * P->mat[1][0] + _tmp0 * _tmp1;
	const float _tmp3 = 1.0f /(_tmp2 + epsilon * (2 * min(0, (((_tmp2) > 0) - ((_tmp2) < 0))) + 1));
	const float _tmp4 = _tmp0 * _tmp3;
	const float _tmp5 = P->mat[1][0] * _tmp3;
	const float _tmp6 = P->mat[0][1] * _tmp3;
	const float _tmp7 = _tmp1 * _tmp3;
	const float _tmp8 = -P->mat[0][1] * _tmp5;
	const float _tmp9 = P->mat[0][0] * _tmp4 + _tmp8;
	const float _tmp10 = -P->mat[1][1] * _tmp5 + _tmp0 * _tmp5;
	const float _tmp11 = P->mat[2][0] * _tmp4 - P->mat[2][1] * _tmp5;
	const float _tmp12 = -P->mat[0][0] * _tmp6 + _tmp1 * _tmp6;
	const float _tmp13 = P->mat[1][1] * _tmp7 + _tmp8;
	const float _tmp14 = -P->mat[2][0] * _tmp6 + P->mat[2][1] * _tmp7;

	e_matrix_t _s_inv;
	InitMatrix(&_s_inv,2,2,0.0f);
	_s_inv.mat[0][0] = _tmp4;
	_s_inv.mat[1][0] = -_tmp5;
	_s_inv.mat[0][1] = -_tmp6;
	_s_inv.mat[1][1] = _tmp7;
	CopyMatrixValues(&_s_inv,S_inv);
	DeInitMatrix(&_s_inv);
    ...
    *S_det_inv = _tmp3;
    ...
    _k.mat[1][1] = _tmp13;
    _k.mat[2][1] = _tmp14;
	CopyMatrixValues(&_k,K);
	DeInitMatrix(&_k);

	e_matrix_t _p_new;
	InitMatrix(&_p_new,3,3,0.0f);
    _p_new.mat[0][0] = -P->mat[0][0] * _tmp9 + P->mat[0][0] - P->mat[1][0] * _tmp12;
    _p_new.mat[1][0] = 0.0f;
    _p_new.mat[2][0] = 0.0f;
    _p_new.mat[0][1] = -P->mat[0][1] * _tmp9 + P->mat[0][1] - P->mat[1][1] * _tmp12;
    _p_new.mat[1][1] = -P->mat[0][1] * _tmp10 - P->mat[1][1] * _tmp13 + P->mat[1][1];
    _p_new.mat[2][1] = 0.0f;
    _p_new.mat[0][2] = -P->mat[0][2] * _tmp9 + P->mat[0][2] - P->mat[1][2] * _tmp12;
    _p_new.mat[1][2] = -P->mat[0][2] * _tmp10 - P->mat[1][2] * _tmp13 + P->mat[1][2];
    _p_new.mat[2][2] = -P->mat[0][2] * _tmp11 - P->mat[1][2] * _tmp14 + P->mat[2][2];
  	CopyMatrixValues(&_p_new,P);
	DeInitMatrix(&_p_new);
